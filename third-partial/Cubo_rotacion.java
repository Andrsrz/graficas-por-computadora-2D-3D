...
public class Cubo_rotacion extends JFrame implements KeyListener {
	private BufferedImage buffer;	
	private Graphics graPixel, gBuffer;
	int ancho = 500, alto = 500;
	int xp = 50, yp=50, zp=100, ra=(int)10, x=(int)200, y=(int)200, z=1;
	...
	public void makeCube(){
		...
		cz[7] += z-ra;
					...
					//rotacion en x 						rotacion en y 							rotacion en z
		/*
		cx[0] = ( (x*1)+(x*0)+(x*0)					*	(x*cosenoy)+(x*0)+(x*senoy)		*	(x*cosenoz)+(x*-senoz)+(x*0) );
		cy[0] = ( (y*0)+(y*cosenox)+(y*-senox)		*	(y*0)+(y*1)+(y*0)				* 	(y*senoz)+(y*cosenoz)+(y*0) );
		cz[0] = ( (z*0)+(z*senox)+(z*cosenox)		*	(z*senoy)+(z*cosenoy)+(z*0)		*	(z*0)+(z*0)+(z*1) );
		
		cx[1] = ( (x*1)+(x*0)+(x*0)					*	(x*cosenoy)+(x*0)+(x*senoy)		*	(x*cosenoz)+(x*-senoz)+(x*0) )+ra;
		cy[1] = ( (y*0)+(y*cosenox)+(y*-senox)		*	(y*0)+(y*1)+(y*0)				* 	(y*senoz)+(y*cosenoz)+(y*0) );
		cz[1] = ( (z*0)+(z*senox)+(z*cosenox)		*	(z*senoy)+(z*cosenoy)+(z*0)		*	(z*0)+(z*0)+(z*1) );

		cx[2] = ( (x*1)+(x*0)+(x*0)					*	(x*cosenoy)+(x*0)+(x*senoy)		*	(x*cosenoz)+(x*-senoz)+(x*0) );
		cy[2] = ( (y*0)+(y*cosenox)+(y*-senox)		*	(y*0)+(y*1)+(y*0)				* 	(y*senoz)+(y*cosenoz)+(y*0) )-ra;
		cz[2] = ( (z*0)+(z*senox)+(z*cosenox)		*	(z*senoy)+(z*cosenoy)+(z*0)		*	(z*0)+(z*0)+(z*1) );

		cx[3] = ( (x*1)+(x*0)+(x*0)					*	(x*cosenoy)+(x*0)+(x*senoy)		*	(x*cosenoz)+(x*-senoz)+(x*0) )+ra;
		cy[3] = ( (y*0)+(y*cosenox)+(y*-senox)		*	(y*0)+(y*1)+(y*0)				* 	(y*senoz)+(y*cosenoz)+(y*0) )-ra;
		cz[3] = ( (z*0)+(z*senox)+(z*cosenox)		*	(z*senoy)+(z*cosenoy)+(z*0)		*	(z*0)+(z*0)+(z*1) );
		
		cx[4] = ( (x*1)+(x*0)+(x*0)					*	(x*cosenoy)+(x*0)+(x*senoy)		*	(x*cosenoz)+(x*-senoz)+(x*0) );
		cy[4] = ( (y*0)+(y*cosenox)+(y*-senox)		*	(y*0)+(y*1)+(y*0)				* 	(y*senoz)+(y*cosenoz)+(y*0) );
		cz[4] = ( (z*0)+(z*senox)+(z*cosenox)		*	(z*senoy)+(z*cosenoy)+(z*0)		*	(z*0)+(z*0)+(z*1) )-ra;

		cx[5] = ( (x*1)+(x*0)+(x*0)					*	(x*cosenoy)+(x*0)+(x*senoy)		*	(x*cosenoz)+(x*-senoz)+(x*0) )+ra;
		cy[5] = ( (y*0)+(y*cosenox)+(y*-senox)		*	(y*0)+(y*1)+(y*0)				* 	(y*senoz)+(y*cosenoz)+(y*0) );
		cz[5] = ( (z*0)+(z*senox)+(z*cosenox)		*	(z*senoy)+(z*cosenoy)+(z*0)		*	(z*0)+(z*0)+(z*1) )-ra;
		
		cx[6] = ( (x*1)+(x*0)+(x*0)					*	(x*cosenoy)+(x*0)+(x*senoy)		*	(x*cosenoz)+(x*-senoz)+(x*0) );
		cy[6] = ( (y*0)+(y*cosenox)+(y*-senox)		*	(y*0)+(y*1)+(y*0)				* 	(y*senoz)+(y*cosenoz)+(y*0) )-ra;
		cz[6] = ( (z*0)+(z*senox)+(z*cosenox)		*	(z*senoy)+(z*cosenoy)+(z*0)		*	(z*0)+(z*0)+(z*1) )-ra;
		
		cx[7] = ( (x*1)+(x*0)+(x*0)					*	(x*cosenoy)+(x*0)+(x*senoy)		*	(x*cosenoz)+(x*-senoz)+(x*0) )+ra;
		cy[7] = ( (y*0)+(y*cosenox)+(y*-senox)		*	(y*0)+(y*1)+(y*0)				* 	(y*senoz)+(y*cosenoz)+(y*0) )-ra;
		cz[7] = ( (z*0)+(z*senox)+(z*cosenox)		*	(z*senoy)+(z*cosenoy)+(z*0)		*	(z*0)+(z*0)+(z*1) )-ra;
		*/
		print();
	}

	public static void main(String[] args) {
		Cubo_rotacion l = new Cubo_rotacion();
		l.makeCube();
	}
